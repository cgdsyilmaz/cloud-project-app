...
class CreateScreenState extends State<CreateScreen> {
  ...
  Future<ShortenedURL> tryShortenURL(String token, String longURL) async {
    ...
    var httpResponse =
        await http.post("$SERVER_ADDRESS_MANAGER/$SHORTEN_URL_PATH",
            headers: {
    ...
    if (httpResponse.statusCode == 200) //login success
    {
      ShortenedURL shortenedURL =
          ShortenedURL.fromJson(json.decode(httpResponse.body));
      _shortURLController.text = shortenedURL.shortURL;
      return shortenedURL;
    } else {
      throw Exception(
          'Failed to get that shortened url for you! Another try maybe?');
    }
  }

  Future<ShortenedURL> tryCustomShortenURL(
      String token, String longURL, String customURL) async {
    var tempToken = token.substring(17, token.length - 2);
    _shortURLController.clear();
    var httpResponse =
        await http.post("$SERVER_ADDRESS_MANAGER/$CUSTOM_SHORTEN_PATH",
            headers: {
              "Authorization": tempToken,
              "Accept": "application/json",
              "content-type": "application/json"
    ...
    if (httpResponse.statusCode == 200) //login success
    {
      ...
      throw Exception(
          'Failed to get that custom shortened url for you! Another one maybe?');
    }
  }
                  ...
                  children: <Widget>[
                    ...
                    FocusScope(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                            hintText: _isCustomURLSelected
                                ? 'Enter your <custom_url> for rdrct.pw/<custom_url>'
                                : 'Your shrtnd URL will magically appear here!'),
                      ),
                    ),
                    FlatButton(
                      onPressed: () => selectDate(context),
                      child: Text('Expiration Date for shortened URL: ' +
                          "${expirationDate.toLocal()}".split(' ')[0]),
                    ),
                    FlatButton(
                      onPressed: () => _isCustomURLSelected
                          ? tryCustomShortenURL(token, _longURLController.text,
