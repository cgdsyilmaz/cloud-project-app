...
class LoginScreen extends StatelessWidget {
  ...
  Future<String> tryLogin(String username, String password) async {
    var httpResult = await http.post('$SERVER_ADDRESS_AUTH/$AUTH_LOGIN_PATH',
        headers: {
  ...
  Future<int> trySignUp(String username, String password) async {
    var httpResult = await http.post('$SERVER_ADDRESS_AUTH/$AUTH_SIGN_UP_PATH',
        headers: {
          "Accept": "application/json",
          "content-type": "application/json"
        },
                ...
                children: <Widget>[
                  ...
                  FlatButton(
                      onPressed: () async {
                        ...
                        if (token != null) {
                          ...
                          displayNotification(context, "An Error Occurred",
                  ...
                  FlatButton(
                      onPressed: () async {
                        ...
                        var httpResultStatusCode =
                            await trySignUp(username, password);
                        if (httpResultStatusCode == 200)
                          displayNotification(context, "Success!",
                              "Account created successfully. Please login.");
                        else if (httpResultStatusCode == 409)
                          displayNotification(context, "Username exists!",
                              "Sign up using different username or login.");
                        else {
                          displayNotification(
                              context, "Error!", "Something has gone wrong!");
                        }
                      },
                      child: Text("Sign Up"))
                ],
